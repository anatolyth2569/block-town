...
func _rancher_scan_barns() -> void:
	...
	for map_cell in gm._buildings:
		...
		if queued_cells.has(barn_cell): continue
		if gm.is_field_job_claimed(barn_cell): continue
		# Priority 1: collect output that's ready
		if barn.get_local_stock_total() > 0:
			_rancher_job_queue.append({"type": "collect", "cell": barn_cell})
			queued_cells.append(barn_cell)
		# Priority 2: deliver feed if barn needs it
		elif barn.data.consumes.has("Feed"):
...
func _on_produce() -> void:
	...
	if active_produces.is_empty():
		...
		# Passive production: output stays in local_stock until a carrier picks it up
		var total_out: int = 0
		for v in active_produces.values(): total_out += v
		...
		for res in active_produces:
			_local_stock[res] = _local_stock.get(res, 0) + active_produces[res]
		_update_indicator(Color(0.1, 0.9, 0.2))
		_last_countdown_secs = -1
		_update_stock_label()
	else:
...
func _worker_road_delivery_cycle() -> Array:
	...
	# Physical goods: factory output → warehouse
	...
	var sto_cell: Vector2i = _find_nearest_warehouse(gm, wcell)

	if sto_cell == Vector2i(-1, -1):
		# No warehouse → drop at nearest road
		for res in _pending_output.keys():
...
func _get_info_text() -> String:
	...
	if active_consumes.size() > 0:
		var parts: Array = []
		for res in active_consumes:
			var have: int = _resource_manager.get_amount(res) if _resource_manager != null else 0
