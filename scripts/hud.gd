...
# [STAGED ROLLOUT] เพิ่ม category เข้ามาทีละหมวด: 0=farm, 1=ranch, 2=industrial, 3=housing, 4=trade
const ACTIVE_CATEGORIES: Array = [0, 1, 2, 3, 4]  # 0=farm, 1=ranch, 2=industrial, 3=housing, 4=trade

const BUILDING_PATHS: Array = [
	"res://resources/buildings/lumberyard.tres",
	"res://resources/buildings/well.tres",
	"res://resources/buildings/pond_large.tres",
	"res://resources/buildings/pond_small.tres",
...
func _build_bottom_bar() -> void:
	...
	for i in range(CAT_LABEL.size()):
		# skip category tabs whose items are not in ACTIVE_CATEGORIES
		if i > 0 and i < 6:
...
func _refresh_store_cards() -> void:
	...
	for path in BUILDING_PATHS:
		...
		if bd == null:
			continue
		if not (int(bd.category) in ACTIVE_CATEGORIES):
			continue
...
func _make_building_preview(bd: BuildingData, bg_col: Color) -> Control:
	...
	sv.own_world_3d = true  # isolate 3D world — prevents models/lights leaking into main scene
	svc.add_child(sv)

	# Environment: background matches card
	var env := Environment.new()
	env.background_mode = Environment.BG_COLOR
	env.background_color = bg_col.lightened(0.55)
	env.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	env.ambient_light_color = Color(0.88, 0.88, 0.88)
	...
	# Key light — safe inside SubViewport (own_world_3d=true isolates it from main scene)
	var key := DirectionalLight3D.new()
	key.rotation_degrees = Vector3(-50, -30, 0)
	key.light_color = Color(1.0, 0.99, 0.94)
