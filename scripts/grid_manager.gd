...
const COLOR_GRASS  := Color(0.20, 0.40, 0.08)
...
func _make_fallback_tree(variant: int) -> Node3D:
	...
	if variant == 0:
		...
		sphere.height = 1.4
		foliage.mesh = sphere
		foliage_mat.albedo_color = Color(0.10, 0.40, 0.10)
		...
		cone.height = 1.9
		foliage.mesh = cone
		foliage_mat.albedo_color = Color(0.06, 0.30, 0.12)
...
func _spawn_pond(cells: Array, coverage: int) -> void:
	...
	var scene_path := "res://scenes/buildings/pond_large.tscn" if coverage >= 2 else "res://scenes/buildings/pond_small.tscn"
	...
	for cell in cells:
		...
		if packed != null:
			...
			inst.name = "tile_%d_%d" % [cell.x, cell.y]
			inst.position = Vector3(cx, 0.0, cz)
			pond_node.add_child(inst)
		else:
			# Fallback procedural pond if scene missing
